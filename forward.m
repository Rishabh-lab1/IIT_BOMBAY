function [Jh,Jv,ro]=forward(yky,t,es,sig,so,nel,akel,opt,tev,k1,x,V1,data,prho,npar,par,p,prho_h,prho_v)
...
if opt==1
    VVT=tt.*yky(1)/pi;
    Jh=jacob(VVT,V1,data,ro,prho_h,npar);
    Jv=jacob(VVT,V1,data,ro,prho_v,npar);
else
    J=0;
    Jh=0;
    Jv=0;
