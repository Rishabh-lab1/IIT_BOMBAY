...
while(norm(gc)/normg0 > para.tol & itc < para.maxit & norm(gc)>1e-20)
  ...
  for ii=1:nf,
    ...
    robsf1=indata((ii-1)*nobs+1:ii*nobs,2);
    robsf2=indata((ii-1)*nobs+1:ii*nobs,3);
    robsf3=indata((ii-1)*nobs+1:ii*nobs,4);
    robsf4=indata((ii-1)*nobs+1:ii*nobs,5);
    
%     robf1=indata1((ii-1)*nobs+1:ii*nobs,2);
%     robf2=indata1((ii-1)*nobs+1:ii*nobs,3);
%     robf3=indata1((ii-1)*nobs+1:ii*nobs,4);
%     robf4=indata1((ii-1)*nobs+1:ii*nobs,5);
    
    
    
%     minobs1(ii)=min(robsf1);
%     maxobs1(ii)=max(robsf1);
%     difobs1(ii)=(maxobs1(ii)-minobs1(ii))/2;
%     minobs2(ii)=min(robsf2);
%     maxobs2(ii)=max(robsf2);
%     difobs2(ii)=(maxobs2(ii)-minobs2(ii))/2;
%     robs11=[robs11;robsf1/difobs1(ii)];
%     robs22=[robs22;robsf2/difobs2(ii)];
    
    robs1((ii-1)*nobs+1:ii*nobs)=robsf1;
    robs2((ii-1)*nobs+1:ii*nobs)=robsf2;
    robs3((ii-1)*nobs+1:ii*nobs)=robsf3;
    robs4((ii-1)*nobs+1:ii*nobs)=robsf4;
    % Forward program to compute response
    
    %fprintf('time taken in forward computaion\n');
    %tic
...
fprintf('%s\n',astr);
   ...
   MTX.mc = xc;
   s = ipcg(A,MTX, para.BETA, -gc, para.intol, para.ininintol, para.init); 
